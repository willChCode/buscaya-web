<template>
  <div class="relative">
    ...
    <div class="flex gap-3 relative z-10">
      ...
      <div 
        v-if="isReply"
        class="absolute -left-[21px] top-[-16px] w-[1.5px] bg-gray-200 z-0 opacity-80"
      ...
      <div class="flex-shrink-0 relative z-20">
        <img 
          v-if="comment.avatar" 
          :src="comment.avatar" 
          :class="[isReply ? 'w-7 h-7' : 'w-10 h-10', 'rounded-full bg-gray-100 border border-gray-100 shadow-sm']"
        />
        <div 
          v-else 
          :class="[isReply ? 'w-7 h-7' : 'w-10 h-10', 'rounded-full bg-gray-50 flex items-center justify-center border border-gray-100']"
        >
      ...
      <div class="flex-1 min-w-0">
        <!-- Text Bubble -->
        <div class="bg-gray-50 rounded-2xl px-4 py-2.5 inline-block max-w-full hover:bg-gray-100/80 transition-colors">
        ...
        <div class="flex items-center gap-4 mt-1.5 ml-2">
          ...
          <button 
            ...
            :class="[
              ...
              comment.isLikedByCurrentUser ? 'text-primary-600' : 'text-gray-500 hover:text-gray-700'
            ]"
          >
            Me gusta
          </button>
          ...
          <div v-if="comment.likes > 0" class="flex items-center gap-1 bg-white shadow-sm border border-gray-100 rounded-full px-1.5 py-0.5 ml-auto">
             <span class="text-[10px]">👍</span>
             <span class="text-[10px] font-bold text-gray-500">{{ comment.likes }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="hasReplies" class="mt-2 relative">
       ...
       <div class="pl-10 space-y-4">
          ...
          >
            ...
            <div 
              class="absolute -left-[21px] top-[-14px] w-[21px] h-[30px] border-l-[1.5px] border-b-[1.5px] border-gray-200 rounded-bl-xl z-0 opacity-80"
            ></div>

            <CommentItem 
              :comment="reply"
              :is-reply="true"
              :is-last="index === displayedReplies.length - 1 && remainingReplies === 0"
          ...
          <div v-if="remainingReplies > 0" class="relative">
            ...
            <div class="absolute -left-[21px] top-[-24px] w-5 h-[40px] border-l-[1.5px] border-b-[1.5px] border-gray-200 rounded-bl-xl z-0 opacity-80"></div>
            
            <button 
              @click="showAllReplies = true"
              class="flex items-center gap-2 pl-2 group h-8"
            >
              <span class="text-[12px] font-bold text-gray-400 group-hover:text-primary-600 transition-colors">
                Ver {{ remainingReplies }} respuesta{{ remainingReplies > 1 ? 's' : '' }} más...
              </span>
            </button>
          </div>
       </div>
...
const displayedReplies = computed(() => {
  ...
  return showAllReplies.value ? props.comment.replies : props.comment.replies.slice(0, 1);
});
