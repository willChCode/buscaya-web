  ...
  <div class="min-h-screen bg-gray-50 pt-4 pb-20">
    <div class="max-w-7xl mx-auto px-4 lg:grid lg:grid-cols-12 lg:gap-8">
      ...
      <main class="lg:col-span-8">
          ...
          >
            <img
              :src="
                ...
                `https://ui-avatars.com/api/?name=${authStore.user?.displayName || 'User'}&background=00cc8d&color=fff`
              "
              class="w-8 h-8 sm:w-10 sm:h-10 rounded-full sm:rounded-2xl object-cover shrink-0"
            />
            <div
              class="flex-1 bg-gray-50 rounded-2xl px-4 py-3 sm:px-5 sm:py-3 text-gray-400 font-medium text-[14px] sm:text-base group-hover:bg-gray-100 transition-colors truncate"
            >
              ¿Qué quieres compartir?
            </div>
            <button
              class="bg-primary-500 text-white p-2 sm:p-3 rounded-2xl shadow-lg shadow-primary-100 group-hover:bg-primary-600 transition-colors shrink-0"
            >
              <svg
        ...
        <div
          class="lg:hidden flex space-x-2 mb-6 overflow-x-auto pb-2 scrollbar-hide"
        >
          <button
            ...
            @click="filter = cat.id"
            :class="[
              'px-4 py-2.5 rounded-full text-[13px] sm:text-sm font-bold transition-all flex items-center gap-2 flex-shrink-0 whitespace-nowrap border shadow-sm',
              filter === cat.id
                ? cat.activeClass
        ...
        <div class="space-y-6">
          ...
          <div
            ...
            class="text-center py-32 bg-white rounded-3xl border border-dashed border-gray-200 shadow-sm"
          >
            <div
              class="inline-flex items-center justify-center w-24 h-24 rounded-full bg-gray-50 mb-6 group-hover:scale-110 transition-transform"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
            ...
            <p class="text-gray-500 max-w-xs mx-auto">
              Sé el primero en compartir algo con tus vecinos de esta zona.
            </p>
            <button
              @click="openCreateModal"
              class="mt-8 bg-primary-500 text-white px-8 py-3 rounded-2xl font-bold hover:bg-primary-600 transition shadow-lg shadow-primary-200"
            >
              Crear publicación
            </button>
          </div>
          ...
          <div v-else class="space-y-6">
            ...
            >
              <div v-if="loadingMore" class="flex justify-center">
                <div
                  class="w-8 h-8 border-4 border-primary-500/20 border-t-primary-500 rounded-full animate-spin"
                ></div>
              </div>
      ...
      <aside class="hidden lg:block lg:col-span-4">
        ...
        >
          ...
          <div class="mx-2">
            <h3
              class="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 pl-4"
            >
              Explorar Temas
            </h3>
            <div class="space-y-1">
              ...
              >
                <div class="flex items-center gap-4 relative z-10">
                  <div
                    ...
                    :class="
                      filter === cat.id
                        ? 'bg-primary-500 text-white shadow-primary-200 rotate-3'
                        : 'bg-gray-100 group-hover:bg-white grayscale'
                    "
                  >
                    {{ cat.emoji }}
                  </div>
          ...
          <div
            class="mx-2 p-6 bg-gradient-to-br from-primary-500 to-primary-600 rounded-[2.5rem] text-white shadow-xl shadow-primary-200 relative overflow-hidden group"
          >
            <div
            ...
            ></div>
            <h4 class="font-black text-lg mb-2 relative z-10">
              ¡Hola, Comunidad!
            </h4>
            <p
              class="text-xs text-white/80 leading-relaxed font-medium relative z-10"
            >
              Comparte noticias, ofertas o simplemente saluda a tus vecinos.
              Juntos hacemos mejor nuestra zona.
            </p>
            <div class="mt-4 flex -space-x-2 relative z-10">
              <div
                v-for="i in 3"
                :key="i"
                class="w-7 h-7 rounded-full border-2 border-primary-500 bg-primary-100 flex items-center justify-center text-[10px]"
              >
                👤
              </div>
              <div
                class="w-7 h-7 rounded-full border-2 border-primary-500 bg-white/20 backdrop-blur-sm flex items-center justify-center text-[8px] font-bold"
              >
                +99
              </div>
            </div>
          ...
          <div class="py-4 text-center">
            <p
              class="text-[10px] text-gray-300 font-bold uppercase tracking-[0.2em]"
            >
              © 2026 Buscaya Comunidad
            </p>
          </div>
        </div>
      </aside>

      <CreatePostModal v-model="showCreateModal" @success="handlePostCreated" />
...
const filterOptions = [
  ...
  },
  {
    ...
    label: 'Ofertas',
    emoji: '🔥',
    activeClass: 'bg-[#F59E0B] border-[#F59E0B] text-white',
  },
  {
    id: 'vecinos',
    label: 'Vecinos',
    emoji: '💬',
    activeClass: 'bg-[#6366F1] border-[#6366F1] text-white',
  },
  {
    id: 'otros',
    label: 'Otros',
...
onMounted(() => {
  if (ubicacionStore.ubicacion) {
    store.fetchPosts(
      ubicacionStore.ubicacion.lat,
      ubicacionStore.ubicacion.lng
    );
  }
});

watch(
  () => ubicacionStore.ubicacion,
  (newVal) => {
    if (newVal) {
...
const handleReportSubmit = async (reason: string) => {
  ...
  if (success) {
    ...
    alert(
      'Reporte enviado con éxito. Gracias por ayudarnos a mantener la comunidad segura.'
    );
  } else {
