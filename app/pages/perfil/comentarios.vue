  ...
  >
    ...
    >
      <div>
        ...
        <p class="text-gray-400 text-sm mt-1">
          Historial de tu participación en la comunidad.
        </p>
      </div>
    </div>
    ...
    <div v-if="comments.length > 0" class="space-y-0">
      ...
      >
        ...
        <div class="flex justify-between items-start">
          <div
            class="flex flex-col sm:flex-row sm:items-center gap-0.5 sm:gap-2 text-xs text-gray-500 mb-1"
          >
            <div class="flex items-center gap-1.5">
              <span class="font-semibold text-gray-900" v-if="comment.isReply"
                >Respuesta</span
              >
              <span class="font-semibold text-gray-900" v-else>Comentario</span>
              <span>en</span>
              <span
                class="font-medium text-gray-700 truncate max-w-[200px] sm:max-w-xs cursor-pointer hover:underline"
          ...
          <div
            class="hidden sm:flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"
          >
            <button
              @click="router.push(`/publicacion/${comment.postId}`)"
        ...
        <!-- Content -->
        <p
          class="text-gray-900 font-medium text-[14px] md:text-[15px] leading-relaxed whitespace-pre-wrap"
        >
          {{ comment.content }}
        </p>
    ...
    >
      ...
      <p class="text-gray-500 max-w-sm">
        Tus comentarios en las publicaciones de la comunidad aparecerán aquí.
      </p>
    </div>
  </div>
</template>
...
const loadMore = async () => {
  ...
  await communityStore.fetchMyComments(
    false,
    communityStore.pageMyComments + 1
  );
...
const confirmDelete = async (comment: any) => {
  ...
  if (confirmed) {
    ...
    if (comment.postId) {
      const success = await communityStore.deleteComment(
        comment.postId,
        comment.id,
        comment.isReply
      );
