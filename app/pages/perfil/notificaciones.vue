<template>
  <div class="bg-white rounded-2xl shadow-sm p-6 sm:p-10 h-full relative min-h-[500px]">
     <!-- Loading Overlay -->
    <div v-if="loading && notifications.length === 0" class="absolute inset-0 bg-white/80 z-10 flex items-center justify-center rounded-2xl">
      <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-500"></div>
    </div>

    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
      <div>
        <h2 class="text-[22px] sm:text-3xl font-bold text-gray-900 tracking-tight">Notificaciones</h2>
        <p class="text-gray-400 text-sm mt-1">Mantente al día con las novedades y alertas.</p>
      </div>
      <button 
       ...
       >
          ...
          <div class="flex-shrink-0 mt-1">
             <div class="w-10 h-10 rounded-full flex items-center justify-center" 
                :class="{
                    'bg-blue-100 text-blue-600': notification.type === 'comment' || notification.type === 'reply',
                    'bg-red-100 text-red-600': notification.type === 'like' || notification.type === 'reaction',
                    'bg-green-100 text-green-600': notification.type === 'system',
                    'bg-gray-100 text-gray-500': !['comment', 'reply', 'like', 'reaction', 'system'].includes(notification.type)
                }"
             >
                <!-- Icons based on type -->
                <svg v-if="notification.type === 'comment' || notification.type === 'reply'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                </svg>
                <svg v-else-if="notification.type === 'like' || notification.type === 'reaction'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="flex-1 min-w-0">
             <div class="flex justify-between items-start">
                 <h4 class="text-sm font-bold text-gray-900 mb-0.5 truncate pr-2" :class="{ 'font-semibold': !notification.read }">
                     {{ notification.title }}
                 </h4>
                 <span class="text-xs text-gray-400 whitespace-nowrap flex-shrink-0">{{ getRelativeTime(notification.createdAt) }}</span>
             </div>
             <p class="text-sm text-gray-600 leading-relaxed">{{ notification.body }}</p>
          </div>

           <!-- Read Indicator -->
           <div v-if="!notification.read" class="w-2 h-2 rounded-full bg-primary-500 mt-2 flex-shrink-0"></div>
       </div>
