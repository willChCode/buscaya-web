<template>
  <div class="bg-white rounded-2xl shadow-sm p-6 sm:p-10 h-full relative min-h-[500px]">
     <!-- Loading Overlay -->
    <div v-if="loading && posts.length === 0" class="absolute inset-0 bg-white/80 z-10 flex items-center justify-center rounded-2xl">
      <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-500"></div>
    </div>

    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
      <div>
        <h2 class="text-[22px] sm:text-3xl font-bold text-gray-900 tracking-tight">Mis Publicaciones</h2>
        <p class="text-gray-400 text-sm mt-1">Gestiona tu actividad en la comunidad.</p>
      </div>
    </div>
    ...
    <div v-if="posts.length > 0" class="space-y-0">
       ...
       >
          <!-- Image -->
          <div class="w-full sm:w-20 sm:h-20 bg-gray-50 rounded-xl overflow-hidden flex-shrink-0 border border-gray-100">
            <img 
              v-if="post.image" 
              :src="post.image" 
          ...
          <div class="flex-1 min-w-0 py-1">
              <div class="flex justify-between items-start">
                  <p class="text-gray-900 font-medium text-[15px] leading-relaxed line-clamp-2 mb-2 pr-8">{{ post.content }}</p>
                  
                  <!-- Desktop Actions (Hover) -->
                  <div class="hidden sm:flex items-center gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                      <button 
              ...
              <div class="flex items-center gap-4 text-xs font-medium text-gray-400 mt-1">
                  <span class="text-gray-500">{{ getRelativeTime(post.timestamp) }}</span>
    ...
    <div v-else-if="!loading" class="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h3 class="text-lg font-bold text-gray-900 mb-2">No tienes publicaciones aún</h3>
      <p class="text-gray-500 max-w-sm mb-6">Empieza a publicar en la comunidad para interactuar con tus vecinos.</p>
      <button 
