<template>
  <Dialog
    ...
    widthClass="w-[98%] max-w-lg mx-auto"
  >
    <div
      class="w-full bg-white rounded-3xl overflow-hidden relative flex flex-col max-h-[90vh]"
    >
      <!-- Overlay for Progress Steps -->
      <div
        v-if="showProgress"
        class="absolute inset-0 z-50 bg-white flex flex-col items-center justify-center p-8 text-center animate-fade-in"
      >
        <!-- Loading Spinner/Icon -->
        <div class="mb-6 relative">
          <div
            class="w-16 h-16 border-4 border-primary-100 border-t-primary-500 rounded-full animate-spin"
          ></div>
          <div class="absolute inset-0 flex items-center justify-center">
            <span class="text-xs font-bold text-primary-600"
              >{{ Math.min((currentStep + 1) * 33, 99) }}%</span
            >
          </div>
        </div>
        ...
        <!-- Step Title -->
        <h3
          class="text-xl font-bold text-gray-900 mb-2 transition-all duration-300"
        >
        ...
        <!-- Step Description -->
        <p
          class="text-gray-500 text-sm mb-6 max-w-xs mx-auto transition-all duration-300"
        >
        ...
        <div
          v-show="currentStep === 1 && !turnstileToken"
          class="min-h-[65px] flex justify-center w-full mb-4"
        >
        ...
        <div class="flex items-center gap-2 mt-8">
          <div
            ...
            :class="
              index <= currentStep ? 'w-8 bg-primary-500' : 'w-2 bg-gray-200'
            "
          ></div>
        </div>
        ...
        <div
          v-if="errorMessage"
          class="text-red-500 text-sm font-medium mt-4 bg-red-50 p-3 rounded-lg border border-red-100 max-w-sm"
        >
          {{ errorMessage }}
          <button
            @click="closeProgress"
            class="block w-full mt-2 text-xs font-bold text-red-600 underline"
          >
      ...
      <div
        ...
        :class="{ 'opacity-0 pointer-events-none': showProgress }"
      >
        <!-- Header with Logo -->
        ...
        <div class="px-6 pb-8 flex-1 overflow-y-auto md:px-8">
          <form @submit.prevent="handleSubmit" class="flex flex-col gap-4">
            ...
            <div
              v-if="errorMessage && !showProgress"
              class="text-red-500 text-sm font-medium text-center bg-red-50 p-3 rounded-lg border border-red-100"
            >
              {{ errorMessage }}
            </div>
            ...
            <div v-if="mode === 'login'" class="flex flex-col gap-3">
              <button
                ...
                class="w-full flex items-center justify-center gap-3 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 font-medium py-3 rounded-xl transition-colors relative group"
              >
                <img
              ...
              <button
                ...
                class="w-full flex items-center justify-center gap-3 bg-black hover:bg-gray-800 text-white font-medium py-3 rounded-xl transition-colors relative group"
              >
                <img
...
const steps = [
  {
    ...
    description: 'Validando que tus credenciales sean correctas...',
  },
  {
    title: 'Escaneando actividad',
    description: 'Protegiendo tu cuenta con Cloudflare...',
  },
  {
    title: 'Iniciando sesión',
    description: 'Consultando base de datos y estableciendo conexión...',
  },
];
...
const handleAppleLogin = async () => {
  if (!(window as any).AppleID) {
    loadAppleSDK();
    errorMessage.value =
      'Iniciando Apple Sign In... intenta de nuevo en un segundo.';
    return;
  }
  ...
  try {
    ...
    if (user) {
      ...
      const decodedToken = JSON.parse(
        atob(authorization.id_token.split('.')[1])
      );
...
const GOOGLE_CLIENT_ID =
  '270578527668-fo3alu7omc8hlca42n349heg9c6u89hu.apps.googleusercontent.com';
