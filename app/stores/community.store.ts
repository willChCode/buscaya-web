...
import type { Post, BackendPost, PostComment, BackendComment } from '@/types/community.types';
  ...
  actions: {
    async fetchPosts(lat: number, lng: number, force = false, targetPage = 1) {
      ...
      try {
        ...
        const mappedPosts: Post[] = rawPosts.map((bp) => ({
            ...
            authorAvatar: bp.usuario.image?.url || `https://ui-avatars.com/api/?name=${bp.usuario.nombre}&background=random`,
            ...
            comments: bp.comentarios ? bp.comentarios.map((c) => this.mapComment(c)) : [],
            rawReactions: bp.reactions || [],
            isLikedByCurrentUser: bp.reactions ? bp.reactions.some(r => {
    ...
    mapComment(c: BackendComment): PostComment {
        return {
            ...
            avatar: c.usuario.image?.url || `https://ui-avatars.com/api/?name=${c.usuario.nombre}&background=random`,
            content: c.texto,
            timestamp: c.fecha,
            likes: c.reactions ? c.reactions.length : 0,
            rawReactions: c.reactions || [],
            isLikedByCurrentUser: c.reactions ? c.reactions.some(r => {
                const rId = r.user.id || (r.user as any)._id;
                return rId === useAuthStore().user?.uid;
            }) : false,
            authorId: c.usuario.id,
            replies: c.respuestas ? c.respuestas.map(r => this.mapComment(r)) : []
        };
    },
    ...
    async toggleReaction(postId: string, type: string) {
       ...
       try {
         const response: any = await $fetch(`${API_BASE}/comunidad/${postId}/reaction`, {
            method: 'POST',
            body: { type },
    ...
    async addComment(postId: string, text: string, parentId?: string) {
        ...
        if (!post && !(this.currentPost && this.currentPost.id === postId)) return false;
        ...
        try {
            ...
            if (parentId) {
                // It's a reply
                const newReplyRaw = response.reply;
                if (!newReplyRaw) {
                    await this.fetchPostById(postId, authStore.user.token); // Fallback
                    return true;
                }
                const newReply = this.mapComment(newReplyRaw);
                
                const findAndAddReply = (comments: PostComment[]): boolean => {
                ...
                let updated = false;
                if (post) updated = findAndAddReply(post.comments) || updated;
                if (this.currentPost && this.currentPost.id === postId) updated = findAndAddReply(this.currentPost.comments) || updated;

                if (!updated) {
                    // Fallback if parent not found in local tree
                    await this.fetchPostById(postId, authStore.user.token);
                }
            } else {
                // It's a top-level comment
                const newCommentRaw = response.comment;
                if (!newCommentRaw) {
                     await this.fetchPostById(postId, authStore.user.token);
                     return true;
                }
    ...
    async toggleCommentReaction(postId: string, commentId: string, type: string = 'like') {
        const config = useRuntimeConfig();
        const API_BASE = config.public.apiBase;
        const authStore = useAuthStore();
        ...
        const findAndToggleComment = (comments: PostComment[], isNested = false): boolean => {
            for (const c of comments) {
                if (c.id === commentId) {
        ...
        if (this.currentPost && this.currentPost.id === postId) updated = findAndToggleComment(this.currentPost.comments) || updated;
        ...
        try {
            ...
            if (this.currentPost && this.currentPost.id === postId) updateCommentStats(this.currentPost.comments);
            
            return true;
        } catch (error) {
            console.error('Error toggling comment/reply reaction (rolling back):', error);
            // 4. Rollback
            if (postIndex !== -1 && post) {
    ...
    async deleteComment(postId: string, commentId: string, isReplyInput?: boolean) {
        const config = useRuntimeConfig();
        const API_BASE = config.public.apiBase;
        const authStore = useAuthStore();
        ...
        if (isReply === undefined) {
             console.warn("deleteComment: Unable to determine if comment or reply. Post not found and isReplyInput not provided.");
             return false;
        }
    ...
    async fetchMyPosts(force = false, targetPage = 1) {
        ...
        try {
            const response: any = await $fetch(`${API_BASE}/comunidad/mis-posts?page=${targetPage}`, {
                headers: {
                    Authorization: `Bearer ${authStore.user.token}`
                }
            });
            ...
            const mappedPosts: Post[] = rawPosts.map((bp: any) => ({
                ...
                authorAvatar: bp.usuario?.image?.url || `https://ui-avatars.com/api/?name=${bp.usuario?.nombre || 'User'}&background=random`,
                timestamp: bp.createdAt,
                category: (bp.categoria || "otros").toLowerCase() as any,
                content: bp.contenido,
                image: bp.imagen?.url,
                likes: bp.reactions ? bp.reactions.length : 0,
                comments: bp.comentarios ? bp.comentarios.map((c: any) => this.mapComment(c)) : [],
                rawReactions: bp.reactions || [],
    ...
    async fetchMyComments(force = false, targetPage = 1) {
        ...
        try {
            const response: any = await $fetch(`${API_BASE}/comunidad/mis-comentarios?page=${targetPage}`, {
                headers: {
                    Authorization: `Bearer ${authStore.user.token}`
                }
            });
